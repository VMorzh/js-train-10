...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const isFrozen = Object.isFrozen(mergedObj);
...
function checkOwnership(obj1, obj2, key) {
  ...
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) {
    // Якщо обидва об'єкти мають вказаний ключ, порівнюємо їх значення.
...
function convertArrayToObj(arr) {
  ...
  // Створюємо пустий об'єкт, в який будемо записувати ключі та значення
  const obj = {};

  // Проходимося по кожному підмасиву в масиві
  for (let i = 0; i < arr.length; i++) {
    // Розпаковуємо підмасив на окремі змінні для ключа та значення
